 ...
 * 系统版本：1.0.2
 * 系统作者：keen
 * 系统说明：本系统基于tinypng压缩，key需要自己去官方申请，进入根目录直接node index，然后输入key值即可自动压缩，config为一些配置，
 * 默认输出目录会在resource，按照放入的原样输出，将需要压缩的图片放到resource目录下，压缩完成后，会自动放回对应目录。
 * 完成时间：2020-05-19
 * 更新时间：2020-06-01
...
class Compress {
    constructor(key, path) {
        console.log('zlife@vip.qq.com：服务已启动...');
        this.source = [];
        this.allnum = 0;
        this.init(key, path);
    }
